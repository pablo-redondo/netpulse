  ...
  if (incidents.length === 0) {
    return (
      <p className="text-sm text-text-muted">
        Sin incidentes registrados en el periodo mostrado.
      </p>
    );
  }
            ...
            <div>
              <div className="flex items-center gap-2 text-sm font-medium text-text-primary">
                <span
                  ...
                  style={{
                    background: isOpen ? 'var(--status-critical)' : 'var(--status-good)',
                  }}
                  aria-hidden
                />
                {showServiceName && `${incident.serviceName} — `}
                {isOpen ? 'Caída en curso' : 'Caída resuelta'}
              </div>
              {incident.cause && (
                <p className="mt-0.5 text-xs text-text-muted">{incident.cause}</p>
              )}
            </div>
            <div className="shrink-0 text-right text-xs text-text-secondary">
              <div>{formatDateTime(incident.startedAt)}</div>
